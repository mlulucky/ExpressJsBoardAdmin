...
class BoardImgsDao{
    ...
    async findByBId(bId){
        const [rows,f]=await pool.query(this.#findByBIdSql,[bId]);
        return rows;
    }
    async deleteById(biId){
        let del=0;
        try {
            const [result]=await pool.execute(this.#deleteByIdSql,[biId]);
    ...
    async insertOne(boardImg){//boardImg : {b_id:"",img_path:""}
        let insert=0;
        try {
            const [result]=await pool.execute(this.#inserOneSql,[boardImg.b_id, boardImg.img_path]);
...
module.exports=new BoardImgsDao();